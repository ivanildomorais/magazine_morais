...
function buildProductDetails(root, productObj){
  ...
  let addToCartTextNode = document.createTextNode('Adicionar ao carrinho');
  ...
  let h3DescTextNode = document.createTextNode('Descrição');
...
function buildComments(root, comment){
  ...
  buscarDados('https://trabalho-de-recuperacao-e1c95-default-rtdb.firebaseio.com/.json').then( data => exibirDados(data) )
