...
let h3TextNode  =document.createTextNode('Você não esta Logado');
h3.appendChild(h3TextNode);
let aBtn = document.createElement('a');
aBtn.classList.add('btn');
aBtn.classList.add('btn-comment');
let btnTextNode = document.createTextNode('Entrar');
...
function buildCartProducts(productObj, root){
  ...
  let deleteBtnTextNode = document.createTextNode('Excluir Carrinho ');
...
let user = firebase.auth().currentUser;
  ...
  let h4QuantityTextNode = document.createTextNode('Quantidade');
  ...
  let quantity = document.createElement('inserir');
...
function buildProceedToCheckOut(list, root){
  ...
  let btnTextNode = document.createTextNode('Finalizar a compra');
